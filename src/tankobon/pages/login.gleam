...
pub type Model {
  Model(form: form.Form(Login), loading: Bool, error: Bool)
}

pub type Msg {
  FormSubmitted(Result(Login, form.Form(Login)))
  LoginResponse(Result(auth.Account, rsvp.Error(String)))
}
...
pub fn element() {
  element.element("login-page", [attribute.class("w-screen h-screen")], [])
}
...
fn init(_) {
  #(Model(form: login_form(), loading: False, error: False), effect.none())
}
  ...
  case msg {
    FormSubmitted(Ok(act)) -> {
      #(
        ...
        auth.login(act.username, act.password, LoginResponse),
      )
    }
    FormSubmitted(Error(f)) -> {
      #(Model(..m, form: f), effect.none())
    }
    LoginResponse(Ok(act)) -> {
